...
const MOCK_COMPANIES = [
  { id:1, name:"PT Maju Bersama", industry:"Manufaktur", periode:"FY 2024", score:78, status:"SEHAT", trend:+5, rasio:{ current:2.1, der:0.8, npm:12.4, roe:18.2, ocf:15.3, cost_rev:64, ebitda:22, quick:1.8, cash:0.9, interest:6.2, dar:0.44, gpm:32, opm:18, roa:14.2, fcf:4.2, capex_r:8, opex_r:18, asset_turn:1.2, fixed_asset_turn:2.8, dso:62, inv_turn:8.2, ap_turn:6.4, dep_r:12, aset_tetap_bruto:42, akumulasi_penyusutan:18, aset_tetap_bersih:24 }, warnings:[{level:"KUNING",cat:"Piutang",msg:"DSO 62 hari, di atas rata-rata industri 45 hari"},{level:"HIJAU",cat:"Likuiditas",msg:"Current ratio 2.1 — posisi kas sangat aman"},{level:"HIJAU",cat:"Aset Tetap",msg:"Aset tetap bersih Rp 24M — kondisi aset baik"}], ringkasan:"PT Maju Bersama menunjukkan kondisi keuangan yang sehat dengan profitabilitas solid.", keputusan:{ investasi:"Ekspansi kapasitas layak — FCF positif Rp 4.2M", pendanaan:"DER 0.8, ruang utang masih ada hingga Rp 12M", dividen:"Dapat distribusi dividen 30-40% dari laba bersih", prioritas:"Percepat collection piutang, target DSO ≤ 45 hari" } },
  { id:2, name:"CV Sentosa Jaya", industry:"Perdagangan", periode:"FY 2024", score:51, status:"WASPADA", trend:-3, rasio:{ current:1.1, der:1.9, npm:4.2, roe:9.1, ocf:6.8, cost_rev:82, ebitda:9, quick:0.9, cash:0.3, interest:2.1, dar:0.66, gpm:18, opm:6, roa:5.1, fcf:-1.2, capex_r:12, opex_r:28, asset_turn:0.9, fixed_asset_turn:1.4, dso:74, inv_turn:4.1, ap_turn:3.8, dep_r:18, aset_tetap_bruto:28, akumulasi_penyusutan:14, aset_tetap_bersih:14 }, warnings:[{level:"MERAH",cat:"Solvabilitas",msg:"DER 1.9 — beban utang terlalu tinggi"},{level:"KUNING",cat:"Profitabilitas",msg:"Net margin 4.2% di bawah industri 6.5%"},{level:"KUNING",cat:"Aset Tetap",msg:"Penyusutan 18%/tahun — evaluasi penggantian aset"}], ringkasan:"CV Sentosa Jaya berada di zona waspada. Tingkat utang tinggi jadi risiko utama.", keputusan:{ investasi:"Tunda ekspansi — perbaiki neraca dulu", pendanaan:"Prioritas lunasi utang berbiaya tinggi", dividen:"Tidak disarankan dividen tahun ini", prioritas:"Kurangi DER ke bawah 1.5 dalam 12 bulan" } },
];
        ...
        <div style={{ display:"flex", flexDirection:"column", flex:1 }}>
          ...
          <div style={{ padding:"24px 32px", flex:1, overflowY:"auto" }}>
            ...
            {detailTab==="rasio" && (
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                {[
                  { title:"💧 Likuiditas", items:[["Current Ratio",selectedCompany.rasio.current+"x",selectedCompany.rasio.current>=1.5],["Quick Ratio",selectedCompany.rasio.quick+"x",selectedCompany.rasio.quick>=1],["Cash Ratio",selectedCompany.rasio.cash+"x",selectedCompany.rasio.cash>=0.5],["OCF Ratio",selectedCompany.rasio.ocf+"x",selectedCompany.rasio.ocf>=1]] },
                  { title:"🏦 Solvabilitas", items:[["Debt/Equity (DER)",selectedCompany.rasio.der+"x",selectedCompany.rasio.der<=1],["Debt/Assets (DAR)",selectedCompany.rasio.dar+"x",selectedCompany.rasio.dar<=0.5],["Interest Coverage",selectedCompany.rasio.interest+"x",selectedCompany.rasio.interest>=3]] },
                  { title:"📈 Profitabilitas", items:[["Gross Margin",selectedCompany.rasio.gpm+"%",selectedCompany.rasio.gpm>=30],["Net Margin",selectedCompany.rasio.npm+"%",selectedCompany.rasio.npm>=8],["ROA",selectedCompany.rasio.roa+"%",selectedCompany.rasio.roa>=10],["ROE",selectedCompany.rasio.roe+"%",selectedCompany.rasio.roe>=15],["EBITDA",selectedCompany.rasio.ebitda+"%",selectedCompany.rasio.ebitda>=15]] },
                  { title:"🏭 Aset Tetap", items:[["Aset Tetap Bruto","Rp "+selectedCompany.rasio.aset_tetap_bruto+"M",true],["Akumulasi Penyusutan","Rp "+selectedCompany.rasio.akumulasi_penyusutan+"M",true],["Aset Tetap Bersih","Rp "+selectedCompany.rasio.aset_tetap_bersih+"M",selectedCompany.rasio.aset_tetap_bersih>=0],["Fixed Asset Turnover",selectedCompany.rasio.fixed_asset_turn+"x",selectedCompany.rasio.fixed_asset_turn>=2],["Depreciation Rate",selectedCompany.rasio.dep_r+"%",selectedCompany.rasio.dep_r<=15]] },
                  { title:"💵 Cash Flow", items:[["OCF/Revenue",selectedCompany.rasio.ocf+"%",selectedCompany.rasio.ocf>=10],["Free Cash Flow","Rp "+selectedCompany.rasio.fcf+"M",selectedCompany.rasio.fcf>=0],["CapEx Ratio",selectedCompany.rasio.capex_r+"%",selectedCompany.rasio.capex_r<=15]] },
                  { title:"🔧 Efisiensi Operasional", items:[["Cost/Revenue",selectedCompany.rasio.cost_rev+"%",selectedCompany.rasio.cost_rev<=70],["OpEx Ratio",selectedCompany.rasio.opex_r+"%",selectedCompany.rasio.opex_r<=25]] },
                  { title:"📦 Aktivitas", items:[["Asset Turnover",selectedCompany.rasio.asset_turn+"x",selectedCompany.rasio.asset_turn>=1],["DSO",selectedCompany.rasio.dso+" hari",selectedCompany.rasio.dso<=45],["Inventory Turnover",selectedCompany.rasio.inv_turn+"x",selectedCompany.rasio.inv_turn>=6],["AP Turnover",selectedCompany.rasio.ap_turn+"x",selectedCompany.rasio.ap_turn>=6]] },
                ].map(s=>(
